...
PORT = 6667
NICK = "pajabotti"
# NICK = "aasdkjaskdjhsskjh"
CHAN = "!4BNSUsik-elepaja"
# CHAN = "#bottitestikanava"
...
swearwords_regex = re.compile(r".*(vi(tt|dd)u|vi[dt]un|perkele|saatana|kuradi|helvett?i|jumalau[td]|kyr[pv]ä|pillu|paska|perse|runkk|homo|kusi?pä).*", flags=re.IGNORECASE)
...
while True:
        ...
        if state == 1:
            ...
            if door_opener == "":
                ...
                irc_action(sock, CHAN, random.choice([
                    "läps nya~ndyn, korjaa jo se saatanan POSIX-putkesta lukeminen",
                    "töks nya~ndyn, tota bugia ei oo vieläkään korjattu",
                ]))
    ...
    if readstate == 1 and state == 0 and timediff > 0:
        ...
        if door_opener is not None and door_timestamp + door_latch_timeout > time.time():
            ...
            subprocess.call("aplay tada.wav", shell=True)
            if door_opener == "":
                irc_say(sock, CHAN, "Riba: andyn ei oo vieläkään jaksanut korjata"
                        "ovilukon putkesta lukemista. potki sitä perseelle tai jotain")
            else:
